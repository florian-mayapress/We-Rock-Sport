...
if ( ! function_exists( 'pixwell_theme_translation_default' ) ) {
	function pixwell_theme_translation_default( $data = array() ) {
		$default = array(
			...
			'read'              => esc_html__( '%s Min Read', 'pixwell' ),
			...
			'share_on'          => esc_html__( 'Share on', 'pixwell' ),
			...
			'share_telegram'    => esc_html__( 'Share on Telegram', 'pixwell' ),
			...
			'updated'           => esc_html__( 'Last Updated:', 'pixwell' ),
		);
...
if ( ! function_exists( 'pixwell_translate' ) ) {
	function pixwell_translate( $text = null ) {
		...
		if ( isset( $translation_data[ $text ] ) ) {
			return apply_filters( 'the_title', $translation_data[ $text ], 12 );
		};
