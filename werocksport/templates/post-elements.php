...
if ( ! function_exists( 'pixwell_post_title' ) ) {
	function pixwell_post_title( $h_tag = 'h2', $bookmark = false, $classes = '' ) {
		...
		<a class="p-url" href="<?php echo esc_url( get_permalink() ); ?>" rel="bookmark" title="<?php echo wp_strip_all_tags( $post_title ); ?>"><?php
			if ( ! empty( $post_title ) ) {
				the_title();
			} else {
				echo get_the_date();
...
if ( ! function_exists( 'pixwell_post_meta_update' ) ) {
	function pixwell_post_meta_update( $post_id = '' ) {
		...
		<span class="meta-info-el meta-info-update">
			<?php if ( ! empty( $meta_date_icon ) ) : ?><i class="rbi rbi-clock"></i><?php endif; ?>
			<?php if ( ! empty( $timestamp ) ) : ?>
				<abbr class="date date-updated" title="<?php echo get_the_modified_date( DATE_W3C, $post_id ); ?>"><?php echo sprintf( pixwell_translate( 'ago' ), human_time_diff( $timestamp, current_time( 'timestamp' ) ) ); ?></abbr>
			<?php else : ?>
